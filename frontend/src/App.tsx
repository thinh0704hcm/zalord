...
function CanaryDemo() {
  ...
  useEffect(() => {
    fetch('/api/v1/messages/canary', { cache: 'no-store' })
      .then((response) => {
        if (!response.ok) throw new Error(`message-service returned ${response.status}`);
        return response.json();
      })
      .then(setPayload)
      .catch((error: unknown) => setError(error instanceof Error ? error.message : 'Canary request failed'));
  }, []);
  ...
  return (
    <main className="min-h-screen bg-[#101820] px-6 py-12 text-[#f7efe5]">
      <section className="mx-auto max-w-2xl rounded-[2rem] border border-[#f2aa4c]/40 bg-[#16222d] p-8 shadow-2xl shadow-black/30">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.4em] text-[#f2aa4c]">Frontend → message-service</p>
        <h1 className="text-4xl font-black tracking-tight">Canary payload</h1>
        <p className="mt-4 text-lg text-[#f7efe5]/80">/api/v1/messages/canary</p>
        <pre className="mt-8 overflow-x-auto rounded-2xl bg-black/35 p-5 text-sm text-[#f2aa4c]">
          {error || JSON.stringify(payload ?? { loading: true }, null, 2)}
        </pre>
      </section>
    </main>
  );
