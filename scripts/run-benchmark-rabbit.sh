#!/usr/bin/env bash
# End-to-end benchmark — RABBITMQ backend only.
# Twin script: scripts/run-benchmark-kafka.sh
#
# Steps:
#   1. Recreate message/chat/notification with EVENT_BUS=rabbitmq + OUTBOX_POLL_MS=50
#   2. Wait /health green
...
#
# Usage:
#   scripts/run-benchmark-rabbit.sh                      # default rate=200, duration=60
#   RATE=500 DURATION=120 scripts/run-benchmark-rabbit.sh
#   RESET=1 scripts/run-benchmark-rabbit.sh              # wipe results file before run
#
...
# RESET=1 wipes prior results. Default is append so you can run rabbit then
# kafka and compare both rows in the same JSON.
...
echo "══════════════════════════════════════════════════════"
echo " RABBITMQ benchmark — rate=${RATE}/s  duration=${DURATION}s"
...
sleep 3   # let consumers fully subscribe
